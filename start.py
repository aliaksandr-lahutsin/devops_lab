...
def get_repo_sha(json_dict):
    print(
        'repo sha: ' 
        + str(json_dict[0].get('commit').get('tree').get('sha')))


def get_owner_email(json_dict):
    print(
        'owner email: '
        + str(json_dict[1].get('commit').get('author').get('email')))


def get_comment_count(json_dict):
    print(
        'comment count: ' 
        + str(json_dict[1].get('commit').get('comment_count')))
...
def get_info(link_string, wget):
    ...
    if (link_string == 'commits'):
        rc = requests.get(
            data['repo_link_commits'], 
            auth=(data['user_name'], data['token']))
        json_dict = rc.json()
        choose_wget(json_dict, wget)
    else:
        rca = requests.get(
            data['repo_link_commit_activity'], 
            auth=(data['user_name'], data['token'])
            )
        json_dict = rca.json()
        choose_wget(json_dict, wget)
